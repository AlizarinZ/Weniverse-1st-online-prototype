...
class Scene {
	constructor(
		...
		this.player = new THREE.Mesh(
			new THREE.SphereGeometry(7, 0.1, 0.1),
			new THREE.MeshPhongMaterial( { color: 0x000000, specular: 0x666666, emissive: 0x222266, shininess: 10, opacity: 0.3, transparent: true,  blending: THREE.AdditiveBlending  } )
			
		);
		this.player.position.set(0, 0, 12);
		let [videoMaterial] = makeVideoTextureAndMaterial("local-video");
		this.playerHead = new THREE.Mesh(
				new THREE.CircleGeometry(6, 32),
				videoMaterial
		);
		this.playerHead.position.set(0, 0, 0);
		...
		this.helperGrid = new THREE.GridHelper(150, 150);
		this.helperGrid.position.y = -10;
